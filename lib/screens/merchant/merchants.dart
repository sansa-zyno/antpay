...
class Merchants extends StatefulWidget {
  ...
  Merchants({required this.category});

  @override
  State<Merchants> createState() => _Merchants2State();
}

class _Merchants2State extends State<Merchants> {
  Future<QuerySnapshot> getmerchants() async {
    QuerySnapshot merchants = await FirebaseFirestore.instance.collection("merchants").where("category", isEqualTo: widget.category).get();
    return merchants;
  }
                              ...
                              Expanded(
                                child: TextField(
                                  ...
                                  decoration: new InputDecoration(
                                      ...
                                      hintStyle: TextStyle(fontFamily: "Nunito", fontSize: 14, color: appColor)),
                                ),
                              ),
                            ],
                          ),
                                          ...
                                          return ListTile(
                                              onTap: () async {
                                                ...
                                                User merchant = User(
                                                    ...
                                                    avatar: snapshot.data!.docs[index]["avatarUrl"]);
                                                appProvider.getChatData(merchant.uid, ConversationType.user, true);
                                                changeScreen(context,
                                                    ChatScreen(me: me, type: ConversationType.user, conversationWith: merchant, conversationId: ""));
                                              },
                                              leading: ClipRRect(
                                                borderRadius: BorderRadius.circular(30),
                                                child: Image.network(
                                                  snapshot.data!.docs[index]["avatarUrl"],
                                                  height: 40,
                                                ),
                                              ),
                                              title: CustomText(
