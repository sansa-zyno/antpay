...
class _SetPinState extends State<SetPin> {
                ...
                child: Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        CustomText(
                          text: "Choose a secure 4-digit PIN.\n Don’t share this Pin with anyone!",
                          size: 12,
                        ),
                        SizedBox(
                          height: 15,
                        ...
                        isLoading
                            ...
                            : GradientButton(
                                ...
                                clrs: activateButton ? [appColor, appColor] : [Color(0xFFF7F3EF), Color(0xFFF7F3EF)],
                                onpressed: () {
  ...
  setPin() async {
    ...
    Response response = await HttpService.postRequest(
        Api.setPin, {"current_pin": userController.getCurrentUser.antpayPin, "pin": pinController.text, "pin_confirmation": pinController.text},
        bearerToken: true, accessToken: appProvider.token);

    if (response.statusCode == 200) {
      Map res = jsonDecode(response.body);
      log(res.toString());
      await FirebaseFirestore.instance.collection("users").doc(userController.getCurrentUser.uid).update({"antpayPin": pinController.text});
      isLoading = false;
      setState(() {});
      AchievementView(
        color: Colors.green,
